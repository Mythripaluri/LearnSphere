        ...
        {steps.map((item) => (
          <React.Fragment key={item.id}>
            ...
            >
              <div
                className={`grid  aspect-square w-[34px] place-items-center rounded-full border-[1px] 
                    ${step === item.id ? "border-teal-50 bg-teal-900 text-teal-50"
                    : "border-richblack-700 bg-richblack-800 text-richblack-300"}
                    ${step > item.id && "bg-teal-50 text-teal-50"}} `}
              >
                {step > item.id ?
            ...
            {item.id !== steps.length && (
              <div
                className={`h-[calc(34px/2)] w-[33%] border-dashed border-b-2 ${step > item.id ? "border-teal-50" : "border-richblack-500"} `}
              >
              </div>
            )}
          </React.Fragment>
