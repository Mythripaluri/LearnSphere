...
const Navbar = () => {
    ...
    // when user click Navbar link then it will hold teal color
    ...
    return (
        <nav className={`z-[10] flex h-14 w-full items-center justify-center border-b-[1px] border-b-richblack-700 text-white translate-y-0 transition-all ${showNavbar} `}>
            ...
            <div className='flex w-11/12 max-w-maxContent items-center justify-between '>
                {/* logo */}
                {/* <Link to="/">
                    <img src={learnSphereLogo} width={160}  loading='lazy' />
                </Link> */}
                <Link to="/" className="flex items-center h-full">
                    <img
                        src={learnSphereLogo}
                        alt="LearnSphere Logo"
                        className="max-h-10 w-auto object-contain" // h-10 = 40px
                        loading="lazy"
                    />
                </Link>
                                    ...
                                    link.title === "Catalog" ? (
                                        <div
                                            className={`group relative flex cursor-pointer items-center gap-1 ${matchRoute("/catalog/:catalogName")
                                                ? "bg-teal-25 text-black rounded-xl p-1 px-3"
                                        ...
                                        <Link to={link?.path}>
                                            <p className={`${matchRoute(link?.path) ? "bg-teal-25 text-black" : "text-richblack-25"} rounded-xl p-1 px-3 `}>
                                                {link.title}
                                            </p>
                                        </Link>)
                                }
                ...
                <div className='flex gap-x-4 items-center'>
                    {
                        user && user?.accountType !== "Instructor" && (
                            <Link to="/dashboard/cart" className="relative">
                                ...
                                {totalItems > 0 && (
                                    <span className="absolute -bottom-2 -right-2 grid h-5 w-5 place-items-center overflow-hidden rounded-full bg-richblack-600 text-center text-xs font-bold text-teal-100">
                                        {totalItems}
                                    </span>
                                )}
                            </Link>
                    ...
                    }
                    {
                        token === null && (
                            <Link to="/login">
                                {/* <button className='border border-richblack-700 bg-richblack-800 px-[12px] py-[8px] text-richblack-100 rounded-md focus:outline-8 outline-teal-50'> */}
                                <button className={` px-[12px] py-[8px] text-richblack-100 rounded-md 
                                 ${matchRoute('/login') ? 'border-[2.5px] border-teal-50' : 'border border-richblack-700 bg-richblack-800'} `}
                                >
                                    Log in
                                </button>
                            </Link>
                    ...
                    }
                    {
                        token === null && (
                            <Link to="/signup">
                                ...
                                <button className={` px-[12px] py-[8px] text-richblack-100 rounded-md 
                                 ${matchRoute('/signup') ? 'border-[2.5px] border-teal-50' : 'border border-richblack-700 bg-richblack-800'} `}
                                >
                                    Sign Up
                                </button>
                            </Link>
